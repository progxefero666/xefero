...
function startApp(THREE,FBXLoader,OBJLoader,MTLLoader,OrbitControls) {
	// @ts-ignore
	let stlmonitorWidth = Math.round($('#containerGameCanvas').width());
	let stlmonitorHeight = 500; //stlmonitorWidth; //stlmonitorWidth	
	
	// @ts-ignore
	let mapMonitorWidth  = Math.round($('#containerMapCanvas').width());
	let mapMonitorHeight = 300;
	
	let objCanvas = document.getElementById("gl_canvas");
	let mapCanvas = document.getElementById("map_canvas");
	
	wglApp = new WebGLAppRace(
